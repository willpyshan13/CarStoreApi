...
docker rmi car-utility-dev
docker build -t car-utility-dev -f ../src/main/docker/dev/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-utility-dev.tar
docker save car-utility-dev -o /var/local/docker-images-tar/car-utility-dev.tar
scp /var/local/docker-images-tar/car-utility-dev.tar root@8.136.105.20:/var/local/docker-images-tar/car-utility-dev.tar
exit
remotessh
ssh root@8.136.105.20 << remotessh
docker stop car-utility-dev
docker rm car-utility-dev
docker rmi car-utility-dev
docker load < /var/local/docker-images-tar/car-utility-dev.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/utility:/var/local/java-log/utility --memory 700m --restart=always --name car-utility-dev --network host car-utility-dev
exit
remotessh
