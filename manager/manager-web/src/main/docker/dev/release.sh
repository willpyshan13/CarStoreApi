...
docker rmi car-manager-dev
docker build -t car-manager-dev -f ../src/main/docker/dev/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-manager-dev.tar
docker save car-manager-dev -o /var/local/docker-images-tar/car-manager-dev.tar
scp /var/local/docker-images-tar/car-manager-dev.tar root@8.136.105.20:/var/local/docker-images-tar/car-manager-dev.tar
exit
remotessh
ssh root@8.136.105.20 << remotessh
docker stop car-manager-dev
docker rm car-manager-dev
docker rmi car-manager-dev
docker load < /var/local/docker-images-tar/car-manager-dev.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/manager:/var/local/java-log/manager --memory 700m --restart=always --name car-manager-dev --network host car-manager-dev
exit
remotessh
